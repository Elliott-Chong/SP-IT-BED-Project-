...
$('document').ready(async () => {
    ...
    let first = window.location.href.split("?")[1].split('&')
    let [brand, keyword] = first
    formData[brand.split('=')[0]] = brand.split('=')[1]
    formData[keyword.split('=')[0]] = keyword.split('=')[1]
    $('#search-result').text(`Search Results for: ${formData.keyword}`)
    if (formData.keyword === '') {
        $('#search-result').text(`Search Results for: ${formData.brand.replace('%20', ' ')}`)
    }
    else if (formData.brand === '') {
        $('#search-result').text(`Search Results for: ${formData.keyword.replace('%20', ' ')}`)
    }
    else {
        $('#search-result').text(`Search Results for: ${formData.keyword.replace('%20', ' ')}(${formData.brand.replace('%20', ' ')})`)
    }
    const body = JSON.stringify(formData)
    let response = await axios.post('http://localhost:5000/product/search', body, config)
    $('#spinner').detach()
    p.appendTo('body')
